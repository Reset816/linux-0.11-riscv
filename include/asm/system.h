...
#define move_to_user_mode() \
    do { \
        uint64_t __mstatus = read_csr(mstatus); \
        __mstatus &= ~(SSTATUS_SPP | SSTATUS_SPIE | SSTATUS_SIE); \
        __mstatus |= (SSTATUS_UPIE | SSTATUS_UIE); \
        write_csr(mstatus, __mstatus); \
    } while (0)

#define sti() set_csr(sstatus, SSTATUS_SIE)
#define cli() clear_csr(sstatus, SSTATUS_SIE)
